...
- `usb_shield_windows/` - main application package
- `usb_shield_windows/main.py` - application entry point
- `usb_shield_windows/watchdog.py` - persistent watchdog helper for always-on protection
...
- Support for packaging as a standalone Windows executable
...
From the nested application folder:
...
This should create a standalone executable under `dist\USB Shield.exe`.

## Notes

- The app requires administrator privileges to modify `HKLM\SYSTEM\CurrentControlSet\Services\UsbStor`.
- The code already includes Windows-specific logic for USB detection, alert sound playback, and app data folder resolution.
- If running as a packaged executable, asset and state file paths are resolved so bundled files are loaded correctly.
